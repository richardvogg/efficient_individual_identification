...
# directory with all videos !BUT ONLY THE VIDEOS!
path_data = "other/videos/"
# directory where the frames are saved (currently only saves positive frames)
dir_save = "other/frames/"
device = "cpu"
num_models = 1
batch_size = 100
...
model.to(device)
model.load_state_dict(torch.load("other/models/binary_model.pth",map_location='cpu'))
model.eval()
models.append(model)
    ...
    while True:
        ...
        if fd == None or len(batch) == batch_size:
            ...
            for x in range(batch.shape[0]):
                # Change here to select also negative frames
                if result[x].item() == 1:
