...
def read_dir(dir, rs, ts, classes):
    lines = load_txt(dir+"labels")

    img = []
    sol = []

    for line in lines:
        im = cv2.imread(dir+"images/"+line[0]+"_"+line[1]+".png")
    
        if im.shape[0] > 224:
            im = cv2.resize(im, (224,224), cv2.INTER_AREA)
        else:
            im = cv2.resize(im, (224,224), cv2.INTER_CUBIC)
        
        img.append( torch.from_numpy(im).permute(2,0,1).astype(float)/255.0 )
        oh = np.zeros(classes)
        oh[int(line[2])] = 1
        sol.append( oh )

    X_train, X_test, y_train, y_test = train_test_split( img, sol, test_size=ts, random_state=rs)
...
def display_im_label(model, test_loader, device):
    fig = plt.figure(figsize=(15, 15))

    rows = 3
    columns = 3
    value = rows*100 + columns*10
    ...
    for i in range(rows*columns):    
        ...
        plt.imshow(sample)
        plt.axis("off")
        plt.title(pred[i].round(2))

def display(val_loss, train_loss, label):
    plt.plot(val_loss)
    plt.plot(train_loss)
    plt.title(label)
...
def testing_ensemble(dataloader, model, device):
    accs = []
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            accs.append(accuracy(y,av_pred/len(model)))
    return  sum(accs)/len(accs), conv
...
data_path = "../data/second_try/ABJ_labeled/"

with open(data_path+"label_balanced1_1", 'r') as f:
    lines = f.readlines()
